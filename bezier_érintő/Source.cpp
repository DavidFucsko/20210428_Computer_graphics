...
void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
		...
		dragged = getActivePoint(myNDCControlPoints, 0.1f, x, window_height - y);
	}
...
void transformPointsToNDC(float windowWidth, float windowHeight) {
	float ndc_x;
	float ndc_y;
	for (auto& p : myControlPoints) {
		ndc_x = (p.x / (windowWidth / 2)) - 1.0f;
		ndc_y = 1.0f - (p.y / (windowHeight / 2));
		myNDCControlPoints.push_back(glm::vec3(ndc_x, ndc_y, 0.0f));
	}
}
...
void cursorPosCallback(GLFWwindow* window, double xPos, double yPos) {
	if (dragged >= 0) {
		...
		myNDCControlPoints.at(dragged).x = xNorm;
		myNDCControlPoints.at(dragged).y = yNorm;
