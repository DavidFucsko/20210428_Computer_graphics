...
GLuint renderingProgramForPoints;
...
GLuint createShaderProgram(const char* vertexShaderFileName, const char* fragmentShaderFileName) {
	...
	string vertShaderStr = readShaderSource(vertexShaderFileName);
	string fragShaderStr = readShaderSource(fragmentShaderFileName);
...
void generatePoints(GLfloat r, GLint num_segment) {
	GLfloat x, y;
	GLfloat alpha = 0.0f;
...
void init(GLFWwindow* window) {
	...
	renderingProgramForPoints = createShaderProgram("vertexShader.glsl", "fragmentShaderForPoints.glsl");
	generatePoints(0.4f, 8);
...
void cleanUpScene() {
	...
	glDeleteProgram(renderingProgramForPoints);
}
...
void display(GLFWwindow* window, double currentTime) {
	...
	glDrawArrays(GL_TRIANGLE_FAN, 0, myPoints.size());

	glUseProgram(renderingProgramForPoints);
	glPointSize(10.0f);
