...
cameraMoving = glm::vec3(0.0f, 0.0f, 0.0f);
...
void computeCameraMatrix() {
	...
	view = glm::lookAt(cameraPos, glm::vec3(0.0f,0.0f,0.0f), up);
...
//	model	= translateM * rotateM * scaleM;
	...
	//
	angle += 0.5*cameraSpeed; // kamera forgás
	...
	for (int i = 0; i < translations.size(); ++i) {
		translateM = glm::translate(glm::mat4(1.0f), translations.at(i));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(translateM));
