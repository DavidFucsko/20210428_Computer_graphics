...
void generatePolygonPoints(GLint numberOfPoints, GLfloat r) {
	GLfloat x, y;
	GLfloat alpha = 0.0f;
	...
	for (int i = 0; i < numberOfPoints + 1; ++i) {
		...
		polygon.push_back({ x, y, 0.0f });
	}
}

void init(GLFWwindow* window) {
	renderingProgram = createShaderProgram();
	generatePolygonPoints(8, 0.2);
	createPolygon();
...
void display(GLFWwindow* window, double currentTime) {
	...
	scaleMatrix = glm::scale(model, glm::vec3(2.0f, 1.0f, 1.0f));
	translateMatrix = glm::translate(model, glm::vec3(-0.5f, 0.0, 0.0));

	transformation = translateMatrix * scaleMatrix;
	glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transformation));
	glLineWidth(2.0f);
	glDrawArrays(GL_LINE_LOOP, 0, polygon.size());

	glm::mat4 model2 = glm::mat4(1.0f);
	scaleMatrix = glm::scale(model2, glm::vec3(1.0f, 2.0f, 1.0f));
	translateMatrix = glm::translate(model2, glm::vec3(0.2, 0.0, 0.0));

	glm::mat4 transformation2(1.0f);

	transformation2 = translateMatrix * scaleMatrix;
	glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transformation2));
	glLineWidth(2.0f);
	glDrawArrays(GL_LINE_LOOP, 0, polygon.size());

	glm::mat4 model3 = glm::mat4(1.0f);
	scaleMatrix = glm::scale(model3, glm::vec3(1.0f, 1.0f, 2.0f));
	translateMatrix = glm::translate(model3, glm::vec3(0.8, 0.0, 0.0));

	glm::mat4 transformation3(1.0f);

	transformation3 = translateMatrix * scaleMatrix;
	glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transformation3));
	glLineWidth(2.0f);
	glDrawArrays(GL_LINE_LOOP, 0, polygon.size());
