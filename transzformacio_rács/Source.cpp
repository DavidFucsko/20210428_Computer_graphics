...
void display(GLFWwindow* window, double currentTime) {
	...
	float newAngle = 0.0f;
	for (int i = 0; i < 4; ++i) {
		float scale = 2.0f - i * 0.07f;
		float angle = glm::degrees(i * 15.0f);
		newAngle = newAngle + angle;
		glm::mat4 model = glm::mat4(1.0f);
		translateMatrix = glm::translate(model, glm::vec3(0.0 + (i * 0.5), 0.0, 0.0));
		scaleMatrix = glm::scale(model, glm::vec3(0.0, 0.0, 1.0f));
		rotateMatrix = glm::rotate(model, 0.0f, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	for (int i = 0; i < 4; ++i) {
		...
		newAngle = newAngle + angle;
		glm::mat4 model = glm::mat4(1.0f);
		translateMatrix = glm::translate(model, glm::vec3(0.0 + (i * 0.5), 0.0 + 0.5, 0.0));
		scaleMatrix = glm::scale(model, glm::vec3(0.0, 0.0, 1.0f));
		rotateMatrix = glm::rotate(model, 0.0f, glm::vec3(0.0f, 0.0f, 1.0f));
